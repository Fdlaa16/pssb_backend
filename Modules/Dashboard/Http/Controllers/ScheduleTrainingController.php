...
class ScheduleTrainingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $scheduleTrainings = ScheduleTraining::query()
                    ->with([
                        'firstClub',
                        'secoundClub',
                        'stadium'
        ...
        if ($request->has('from_date') && !empty($request->from_date)) {
            $scheduleTrainings->where('created_at', '>=', Helper::formatDate($request->from_date, 'Y-m-d') . ' 00:00:00');
        }
        
        if ($request->has('to_date') && !empty($request->to_date)) {
            $scheduleTrainings->where('created_at', '<=', Helper::formatDate($request->to_date, 'Y-m-d') . ' 23:59:59');
        }

        $scheduleTrainings = $scheduleTrainings->paginate(10);
